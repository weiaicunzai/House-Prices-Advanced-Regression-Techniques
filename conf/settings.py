from datetime import datetime

#voc2012 bgr
#MEAN = (0.40450239443559777, 0.4373051208637668, 0.45734658153594476)
#STD = (0.2846743681700796, 0.27163815793569834, 0.2747289066704502)

# camvid bgr
MEAN = (0.42019099703461577, 0.41323568513979647, 0.4010048431259079)
STD = (0.30598050258519743, 0.3089986932156864, 0.3054061869915674)

CHECKPOINT_FOLDER = '/data/ssd1/xuxinan/mm/checkpoints'
LOG_FOLDER ='runs'

DATE_FORMAT = '%A_%d_%B_%Y_%Hh_%Mm_%Ss'
#time of we run the script
TIME_NOW = datetime.now().strftime(DATE_FORMAT)

#save weights file per SAVE_EPOCH epoch
SAVE_EPOCH = 1000

DATA_PATH = '/content/drive/My Drive/dataset/camvid'

#IMAGE_SIZE = (480, 360)
IMAGE_SIZE = 473
BASE_SIZE = 512
#IMAGE_SIZE = 128
#BASE_SIZE = 138

MILESTONES = [100, 150]
SCALES = [0.5, 0.75, 1.0, 1.25, 1.5, 1.75]

IGNORE_LABEL = 255

EVAL_PATH = 'result'

#CROP_SIZE_GLAS = (416, 416)
# CROP_SIZE_CRAG = (512, 512)
CROP_SIZE_CRAG = (768, 768)
# CROP_SIZE_CRAG = (1024, 1024)
CROP_SIZE_GLAS = (480, 480)