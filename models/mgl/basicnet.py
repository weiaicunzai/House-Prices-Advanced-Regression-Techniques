...
class ConcatNet(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4, size=None):
        # print(x1.shape)
        if size is not None:
            self.h, self.w = size
            #print(size)
...
class GraphNet(nn.Module):
    ...
    def gen_soft_assign(self, x, sigma):
        ...
        # print(soft_assign.shape, '3333') # [batch_size, 3 num_nodes, 32, N 3600]
        # print(soft_assign.shape, x.shape, 1111)
        for node_id in range(self.node_num):
            # print(x.shape,  x.view(B,C,-1).permute(0, 2, 1).contiguous().shape, self.anchor[node_id, :].shape)
            residual = (x.view(B, C, -1).permute(0, 2, 1).contiguous() - self.anchor[node_id, :]).div(sigma[node_id, :]) # + eps)
            # qik is the softassignment of a feature vector fi to wk,
            # and can be computed by the following equation
            soft_assign[:, node_id, :] = -torch.pow(torch.norm(residual, dim=2), 2) / 2

        # print(soft_assign.shape, 'before')
        soft_assign = F.softmax(soft_assign, dim=1) # [3, 32, 3600]
        # print(soft_assign, 11)
        # print(soft_assign.shape, 'after')
        # print(soft_assign.shape, 33333)
    ...
    def forward(self, x):
        ...
        sigma = torch.sigmoid(self.sigma)  # sigma 32(num_nodes), 512 (node_dim)
        # print(sigma.shape)
...
class MutualModule0(nn.Module):
    ...
    def forward(self, edge_graph, region_graph1, region_graph2, assign):
        ...
        #print(edge_x.shape, edge_graph.shape, assign.shape)
        edge_x = self.conv(edge_x.unsqueeze(3)).squeeze(3)
        return edge_x

    def corr_matrix(self, edge, region1, region2):
        #then, thesimilarity matrixAinterC→E∈RK×Kis calculated by a matrixmultiplication as
...
class ECGraphNet(nn.Module):
    ...
    def forward(self, x, edge):
        ...
        # Then, as shownin Figure 4, we obtain the edge-related
        # features on regulargrids ofF′Cin a ‘soft’ manner with
        # the attention mecha
        x1 = torch.sigmoid(edge).mul(x) #elementwise-mutiply
        x1 = self.conv0(x1)

        # Finally, a graph projection opera-tionfGprojis used to transformFeintozedge-based
        # nodeembeddings,
        # denoted asPe={pe1,· · ·, pez}, to representthe edge prior.
...
class MutualModule1(nn.Module):
    ...
    def forward(self, region_x, region_graph, assign, edge_x):
        ...
        # print(region_x.shape, edge_x.shape)
...
class MutualNet(nn.Module):
    ...
    # Cross-Graph InteractionfCGI
    def __init__(self, BatchNorm=nn.BatchNorm2d, dim=512, num_clusters=8, dropout=0.1):
        super(MutualNet, self).__init__()

        self.dim = dim # 512

        #print(num_clusters,  self.dim, '4444444444')
        # num_cluster 32,  dim 512
        self.edge_proj0   = GraphNet(node_num=num_clusters, dim=self.dim, normalize_input=False) # num_clusters 32
    ...
    def forward(self, edge_x, region_x):
        #print(edge_x.shape, region_x.shape)
        # project region/edge fature to graph
        # print(region_x.shape, 333333)
        region_graph, region_assign = self.region_proj0(region_x)
        #print(region_assign.shape,  region_assign[0][1], region_assign[0][:][1].sum())
        # print(region_assign[0][:, 1].sum(), region_assign[0][1, :].sum())
        # print(region_assign.shape)
        edge_graph, edge_assign = self.edge_proj0(edge_x)
        # print(torch.unique(edge_assign))

        # print('ccc', edge_graph.shape)

        #  Cross-Graph Interaction ()
        edge_graph = self.edge_conv(edge_graph.unsqueeze(3)).squeeze(3)
        # print('xxxxxxx', edge_graph.shape)
        ...
        # CGI
        # print(edge_graph.shape)
        # print(region_graph1.shape, 'fffff')
        # 512, 32
        # 512, 32
        # print(edge_assign.shape, 'ccccc')

        # correlation between edge and region
        #print(edge_graph.shape)
        n_edge_x = self.r2e(edge_graph, region_graph1, region_graph2, edge_assign)
        #print(n_edge_x.shape, 11111)

        # After that, we can transfer seman-tic information fromVCtoVEby
        # eq 5
        ...
        #print(region_x.shape, region.shape, edge_x.shape, edge.shape)
