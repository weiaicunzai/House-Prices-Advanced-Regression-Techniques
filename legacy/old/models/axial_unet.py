...
class AxialAttention(nn.Module):
    def __init__(self, in_planes, out_planes, groups=8, kernel_size=56,
        ...
        print(in_planes, out_planes, groups)
...
class ResAxialAttentionUNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2, zero_init_residual=True,
        ...
        self.layer1 = self._make_layer(block, int(128 * s), layers[0], kernel_size=237)
        self.layer2 = self._make_layer(block, int(256 * s), layers[1], stride=2, kernel_size=237,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, int(512 * s), layers[2], stride=2, kernel_size=118,
                                       dilate=replace_stride_with_dilation[1])
        self.layer4 = self._make_layer(block, int(1024 * s), layers[3], stride=2, kernel_size=59,
    ...
    def _make_layer(self, block, planes, blocks, kernel_size=56, stride=1, dilate=False):
        ...
        if stride != 1:
            #print(stride, 'stride')
            kernel_size = math.floor(kernel_size // 2)
            #print(kernel_size)
    ...
    def _forward_impl(self, x):
        ...
        x = F.relu(F.interpolate(self.decoder1(x4), size=x4.shape[-2:], mode ='bilinear'))
        #print(139409, x.shape, x4.shape)
        #x4 = F.interpolate(x4, size=x.shape[-2:])
        x = torch.add(x, x4)
        x = F.relu(F.interpolate(self.decoder2(x) , size=x3.shape[-2:], mode ='bilinear'))
        #x3 = F.interpolate(x3, size=x.shape[-2:])
        x = torch.add(x, x3)
        x = F.relu(F.interpolate(self.decoder3(x) , size=x2.shape[-2:], mode ='bilinear'))
        #x2 = F.interpolate(x2, size=x.shape[-2:])
        x = torch.add(x, x2)
        x = F.relu(F.interpolate(self.decoder4(x) , size=x1.shape[-2:], mode ='bilinear'))
        #x1 = F.interpolate(x1, size=x.shape[-2:])
        x = torch.add(x, x1)
        x = F.relu(F.interpolate(self.decoder5(x) , size=src_shape[-2:], mode ='bilinear'))
...
class medt_net(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, kernel_size=56, stride=1, dilate=False):
        ...
        for _ in range(1, blocks):
            #print(self.inplanes, planes, self.base_width, kernel_size)
            layers.append(block(self.inplanes, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
                                norm_layer=norm_layer, kernel_size=kernel_size))
...
def axialunet(pretrained=False, **kwargs):
    ...
    model = ResAxialAttentionUNet(AxialBlock, [1, 2, 4, 1], s=1/16, **kwargs)
    return model
